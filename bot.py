...
from database import init_db, save_scammer, get_all_scammers, update_payment_status, get_ref_code, create_ref_link, get_referral_stats, create_task, submit_task_proof, approve_task
...
async def show_catalog(query):
    ...
    for cat_id, cat_data in list(catalog.items())[:15]:  # Первые 15
...
async def admin_approve_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if approve_task(task_id):
        await query.edit_message_caption(
            caption=query.message.caption + "\n\n✅ ЗАДАНИЕ ОДОБРЕНО! 0.3 USDT начислено."
        )
    else:
        await query.edit_message_caption(
            caption=query.message.caption + "\n\n❌ ОШИБКА"
